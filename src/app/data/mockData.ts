...
// Engineering branches and their subjects
...
const studentNames = [
  'John Smith', 'Emma Johnson', 'Oliver Williams', 'Sophia Brown', 'Liam Davis',
  'Ava Wilson', 'Noah Martinez', 'Isabella Anderson', 'Ethan Thomas', 'Mia Jackson',
  'Lucas White', 'Charlotte Harris', 'Mason Clark', 'Amelia Lewis', 'Logan Walker',
  'Harper Hall', 'Alexander Allen', 'Evelyn Young', 'Jacob King', 'Abigail Wright',
  'Michael Scott', 'Emily Green', 'Daniel Adams', 'Elizabeth Baker', 'Matthew Nelson',
  'Sofia Carter', 'David Mitchell', 'Avery Perez', 'Joseph Roberts', 'Ella Turner',
  'Samuel Phillips', 'Scarlett Campbell', 'Henry Parker', 'Grace Evans', 'Owen Edwards',
  'Chloe Collins', 'Sebastian Stewart', 'Victoria Morris', 'Jack Rogers', 'Lily Reed',
  'Benjamin Cook', 'Zoey Morgan', 'Theodore Bell', 'Nora Murphy', 'Elijah Bailey',
  'Hannah Rivera', 'Aiden Cooper', 'Lillian Richardson', 'Jackson Cox', 'Addison Howard',
  'Gabriel Ward', 'Natalie Torres', 'Carter Peterson', 'Aubrey Gray', 'Wyatt Ramirez',
  'Brooklyn James', 'Julian Watson', 'Penelope Brooks', 'Luke Kelly', 'Layla Sanders',
  'Anthony Price', 'Zoe Bennett', 'Isaac Wood', 'Leah Barnes', 'Ryan Ross',
  'Savannah Henderson', 'Nathan Coleman', 'Audrey Jenkins', 'Dylan Perry', 'Claire Powell',
  'Caleb Long', 'Paisley Patterson', 'Joshua Hughes', 'Bella Flores', 'Andrew Washington',
  'Skylar Butler', 'Christopher Simmons', 'Hannah Foster', 'Josiah Gonzales', 'Aria Bryant',
  'Hunter Alexander', 'Anna Russell', 'Eli Griffin', 'Caroline Diaz', 'Christian Hayes',
  'Genesis Myers', 'Aaron Ford', 'Kennedy Hamilton', 'Thomas Graham', 'Madelyn Sullivan',
  'Charles Wallace', 'Kinsley Woods', 'Jaxon Cole', 'Naomi West', 'Isaiah Jordan',
  'Elena Owens', 'Landon Reynolds', 'Sarah Fisher', 'Cameron Ellis', 'Maya Gibson',
  'Connor Marshall', 'Ruby Wallace', 'Jeremiah Holmes', 'Alice Stevens', 'Adrian Chapman',
  'Violet Mason', 'Hudson Hunt', 'Samantha Hunter', 'Robert Cole', 'Ariana Webb'
];
...
const generateStudents = (): Student[] => {
  ...
  Object.keys(engineeringBranches).forEach((branch) => {
    const subjects = engineeringBranches[branch as keyof typeof engineeringBranches];
    
    for (let i = 0; i < 20; i++) {
      const name = studentNames[nameIndex % studentNames.length];
      const year = Math.floor(i / 5) + 1; // Distribute across 4 years
      ...
      students.push({
        ...
        guardianContact: `+1${String(Math.floor(Math.random() * 9000000000) + 1000000000)}`,
...
// Generate teachers for each branch
const teacherNames = [
  'Dr. Sarah Miller', 'Prof. John Anderson', 'Dr. Emily Taylor', 'Prof. Robert Clark',
  'Dr. Michael Brown', 'Prof. Jennifer Wilson', 'Dr. David Moore', 'Prof. Linda Garcia',
  'Dr. James Rodriguez', 'Prof. Patricia Martinez', 'Dr. Richard Lee', 'Prof. Mary Lopez',
  'Dr. Thomas White', 'Prof. Barbara Hall', 'Dr. Christopher Allen', 'Prof. Susan Wright',
  'Dr. Daniel King', 'Prof. Karen Scott', 'Dr. Matthew Green', 'Prof. Nancy Adams'
];
...
export const mockAttendance: Attendance[] = mockStudents.slice(0, 30).map((student, index) => ({
  id: `A${String(index + 1).padStart(3, '0')}`,
  studentId: student.id,
  studentName: student.name,
  date: '2026-02-21',
  status: ['Present', 'Present', 'Present', 'Absent', 'Late'][Math.floor(Math.random() * 5)] as 'Present' | 'Absent' | 'Late',
  class: student.class,
  degree: student.degree,
  branch: student.branch
}));

export const mockGrades: Grade[] = mockStudents.slice(0, 20).flatMap((student, studentIndex) => 
  student.subjects.slice(0, 2).map((subject, subjectIndex) => {
...
export const mockSchedules: Schedule[] = Object.keys(engineeringBranches).flatMap((branch, branchIndex) => {
  const subjects = engineeringBranches[branch as keyof typeof engineeringBranches];
